...
/* =================================================================== */
/* CONFIGURAÇÕES DE FREQUÊNCIA EXYNOS 990 (OC)             */
/* Altere os valores _NEW e _MHZ para definir seu overclock/underclock. */
/* =================================================================== */

// --- CPU Cluster 2 (Big Core - M5) ---
#define CPUCL2_FREQ_OLD     3016000     // Frequência original (kHz) na tabela DVFS
#define CPUCL2_FREQ_NEW     3016000     // Nova frequência (kHz) para DVFS
#define CPUCL2_PLL_OLD      3016000000  // Frequência original (Hz) na tabela PLL
#define CPUCL2_PLL_NEW      3016000000  // Nova frequência (Hz) para PLL
#define CPUCL2_VOLT_OLD     3016        // Frequência (MHz) na tabela de Voltagem
#define CPUCL2_VOLT_NEW     3016        // Nova frequência (MHz) para Voltagem
#define CPUCL2_M_VALUE_NEW  359         // Novo valor M para PLL_CPUCL2
#define CPUCL2_K_VALUE_NEW  0       // Novo valor K para PLL_CPUCL2

// --- CPU Cluster 1 (Medium Core - A76) ---
#define CPUCL1_FREQ_OLD     2600000
#define CPUCL1_FREQ_NEW     2600000
#define CPUCL1_PLL_OLD      2600000000
#define CPUCL1_PLL_NEW      2600000000
#define CPUCL1_VOLT_OLD     2600
#define CPUCL1_VOLT_NEW     2600
#define CPUCL1_M_VALUE_NEW  315
#define CPUCL1_K_VALUE_NEW  0

// --- CPU Cluster 0 (Little Core - A55) ---
#define CPUCL0_FREQ_OLD     2106000
#define CPUCL0_FREQ_NEW     2106000
#define CPUCL0_PLL_OLD      2106000000
#define CPUCL0_PLL_NEW      2106000000
#define CPUCL0_VOLT_OLD     2106
#define CPUCL0_VOLT_NEW     2106
#define CPUCL0_M_VALUE_NEW  255
#define CPUCL0_K_VALUE_NEW  0

// --- GPU (G3D - Mali-G77 MP11) ---
#define G3D_FREQ_OLD        897000
#define G3D_FREQ_NEW        921000
#define G3D_PLL_OLD         897000000
#define G3D_PLL_NEW         921000000
#define G3D_VOLT_OLD        897
#define G3D_VOLT_NEW        921
#define G3D_P_VALUE_NEW     4           // Novo valor P para PLL_G3D
#define G3D_M_VALUE_NEW     146         // Novo valor M para PLL_G3D
#define G3D_K_VALUE_NEW     0           // Novo valor K para PLL_G3D

// --- Limites Globais (usados em ect_parse_gen_param_table) ---
#define GLOBAL_MAX_MHZ      (CPUCL2_FREQ_NEW / 1000)
#define GPU_MAX_MHZ         (G3D_FREQ_NEW / 1000)
      ...
      u32 *level_list;
       ...
       if (strcmp(domain->domain_name, "CPUCL0") == 0) {
           ...
           for (j = 0; j < domain->num_of_level; j++) {
               if (level_list[j] == CPUCL0_FREQ_OLD) {
                   level_list[j] = CPUCL0_FREQ_NEW;
                   break;
               }
           }
           ...
           level_list = (u32 *)domain->list_level;
           for (j = 0; j < domain->num_of_level; j++) {
               if (level_list[j] == CPUCL1_FREQ_OLD) {
                   level_list[j] = CPUCL1_FREQ_NEW;
                   break;
               }
           }
           ...
           level_list = (u32 *)domain->list_level;
           for (j = 0; j < domain->num_of_level; j++) {
               if (level_list[j] == CPUCL2_FREQ_OLD) {
                   level_list[j] = CPUCL2_FREQ_NEW;
                   break;
               }
           }
       }
           ...
           level_list = (u32 *)domain->list_level;
           for (j = 0; j < domain->num_of_level; j++) {
               if (level_list[j] == G3D_FREQ_OLD) {
                   level_list[j] = G3D_FREQ_NEW;
                   break;
               }
           }
       }
   ...
   if (ect_pll->pll_name != NULL) {
       ...
       if (strcmp(ect_pll->pll_name, "PLL_CPUCL0") == 0) {
           for (j = 0; j < ect_pll->num_of_frequency * 5; j += 5) {
               if (freq_list[j] == CPUCL0_PLL_OLD) {
                   freq_list[j] = CPUCL0_PLL_NEW;
                   freq_list[j+2] = CPUCL0_M_VALUE_NEW; // M
                   freq_list[j+4] = CPUCL0_K_VALUE_NEW;
                   // K
                   break;
               }
           }
           ...
           for (j = 0; j < ect_pll->num_of_frequency * 5; j += 5) {
               if (freq_list[j] == CPUCL1_PLL_OLD) {
                   freq_list[j] = 
                   CPUCL1_PLL_NEW;
                   freq_list[j+2] = CPUCL1_M_VALUE_NEW; // M
                   freq_list[j+4] = CPUCL1_K_VALUE_NEW;
                   // K
                   break;
               }
           }
           ...
           for (j = 0; j < ect_pll->num_of_frequency * 5; j += 5) {
               if (freq_list[j] == CPUCL2_PLL_OLD) {
                   freq_list[j] = 
                   CPUCL2_PLL_NEW;
                   freq_list[j+2] = CPUCL2_M_VALUE_NEW;   // M
                   freq_list[j+4] = CPUCL2_K_VALUE_NEW;
                   // K
                   break;
               }
           }
           ...
           for (j = 0; j < ect_pll->num_of_frequency * 5; j += 5) {
               if (freq_list[j] == G3D_PLL_OLD) {
             
                   freq_list[j] = G3D_PLL_NEW;
                   freq_list[j+1] = G3D_P_VALUE_NEW;
                   // P
                   freq_list[j+2] = G3D_M_VALUE_NEW;
                   // M
                   freq_list[j+4] = G3D_K_VALUE_NEW;
                   // K
                   break;
               }
           }
...
static int ect_parse_voltage_domain(int parser_version, void *address, struct ect_voltage_domain *domain)
{
   ...
   if (domain->domain_name != NULL) {
       if (strcmp(domain->domain_name, "CPUCL0") == 0) {
           for (j = 0; j < domain->num_of_level; j++) {
               if (domain->level_list[j] == CPUCL0_VOLT_OLD) {
                   domain->level_list[j] = CPUCL0_VOLT_NEW;
                   break;
               }
           }
       } else if (strcmp(domain->domain_name, "CPUCL1") == 0) {
           for (j = 0; j < domain->num_of_level; j++) {
               if (domain->level_list[j] == CPUCL1_VOLT_OLD) {
                   domain->level_list[j] = CPUCL1_VOLT_NEW;
                   break;
               }
           }
       } else if (strcmp(domain->domain_name, "CPUCL2") == 0) {
           for (j = 0; j < domain->num_of_level; j++) {
               if (domain->level_list[j] == CPUCL2_VOLT_OLD) {
                   domain->level_list[j] = CPUCL2_VOLT_NEW;
                   break;
               }
           }
       } else if (strcmp(domain->domain_name, "G3D") == 0) {
           for (j = 0; j < domain->num_of_level; j++) {
               if (domain->level_list[j] == G3D_VOLT_OLD) {
                   domain->level_list[j] = G3D_VOLT_NEW;
                   break;
               }
           }
       }
   ...
   if (function->function_name != NULL) {
       if (strcmp(function->function_name, "LITTLE") == 0) {
           for (j = 0; j < function->num_of_range; j++) {
               ...
               range->max_frequency = CPUCL0_FREQ_NEW; // 2210 MHz in kHz
           }
       }
   }
...
static int ect_parse_gen_param_table(int parser_version, void *address, struct ect_gen_param_table *size)
{
       ...
       ect_parse_integer(&address, &size->num_of_col);
        ...
        for (i = 0; i < size->num_of_row; ++i) {
                int max_clock;
                if (strcmp(size->table_name, "MINMAX_CPUCL0") == 0) {
                        ...
                        if (GLOBAL_MAX_MHZ < max_clock)
                                max_clock = GLOBAL_MAX_MHZ;
                        size->parameter[i * size->num_of_col + MINMAX_BOOT_FREQ] = max_clock;
                } else if (strcmp(size->table_name, "MINMAX_CPUCL1") == 0) {
                        ...
                        if (GLOBAL_MAX_MHZ < max_clock)
                                max_clock = GLOBAL_MAX_MHZ;
                        size->parameter[i * size->num_of_col + MINMAX_BOOT_FREQ] = max_clock;
                } else if (strcmp(size->table_name, "MINMAX_CPUCL2") == 0) {
                        ...
                        if (GLOBAL_MAX_MHZ < max_clock)
                                max_clock = GLOBAL_MAX_MHZ;
                        size->parameter[i * size->num_of_col + MINMAX_BOOT_FREQ] = max_clock;
                } else if (strcmp(size->table_name, "MINMAX_MIF") == 0) {
...
dump_open,
                ...
                .block_name_length = sizeof(BLOCK_TIMING_PARAM) - 
1,
...
static struct ect_info* ect_get_info(char *block_name)
{
        ...
        for (i = 0; i < ARRAY_SIZE32(ect_list); ++i) {
                if (strcmp(block_name, ect_list[i].block_name) == 0)
                        return &ect_list[i];
        }
...
struct ect_dvfs_domain *ect_dvfs_get_domain(void *block, char *domain_name)
{
        ...
        for (i = 0; i < header->num_of_domain; ++i) {
                ...
                if (strcmp(domain_name, domain->domain_name) == 0)
                        return domain;
        }
...
struct ect_pll *ect_pll_get_pll(void *block, char *pll_name)
{
        ...
        for (i = 0; i < header->num_of_pll; ++i) {
                ...
                if (strcmp(pll_name, pll->pll_name) == 0)
                        return pll;
        }
...
struct ect_ap_thermal_function *ect_ap_thermal_get_function(void *block, char *function_name)
{
        ...
        for (i = 0; i < header->num_of_function; ++i) {
                ...
                if (strcmp(function_name, function->function_name) == 0)
                        return function;
        }
...
struct ect_pidtm_block *ect_pidtm_get_block(void *block, char *block_name)
{
        ...
        for (i = 0; i < header->num_of_block; ++i) {
                ...
                if (strcmp(block_name, pidtm_block->block_name) == 0)
                        return pidtm_block;
        }
...
struct ect_bin *ect_binary_get_bin(void *block, char *binary_name)
{
        ...
        for (i = 0; i < header->num_of_binary; ++i) {
                ...
                if (strcmp(bin->binary_name, binary_name) == 0)
                        return bin;
        }
